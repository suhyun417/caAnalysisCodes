...
% 2020/04/09 SHP
%
% 2020/04/22 SHP: from the 2nd session onwards, use the parameters from 1st session
...
for iSubj = 2 %1:2
    ...
    for iSession = 9 %2:length(setDateSession) %startSession:length(setDateSession)
        ...
        if isempty(dir(fullfile(p, [n '_source_extraction'], 'Sources2D_all*.mat'))) %~exist(fullfile(p, [n '_source_extraction']), 'dir')
            ...
            if iSession == 1
                ...
                file_path = fullfile(neuron.P.log_folder,  ['Sources2D_all_nobatch_residualOn_constantParam', strrep(get_date(), ' ', '_'), '.mat']);
            ...
            else % use the first session's parameters
                
                tempDir = sprintf('/procdata/parksh/_marmoset/invivoCalciumImaging/%s/Session/%s/_preproc/ConcatRuns_all_source_extraction', nameSubj, setDateSession{1});
                d_param = dir(fullfile(tempDir, 'Sources2D_all*'));
                load(fullfile(d_param(1).folder, d_param(1).name), 'paramCNMFE');
