...
%         load(fullfile(dirProcdata_session, 'DFL_ts.mat'))
        ...
        if flagPlot % genFig_stimSortedTS_BPM.m does the same plotting
            ...
            setCond = [1 2 5 6 10]; % default set of conditions we want to plot
            nImage = 5;
            
            catCondMat = cat(1, stimTiming_BPM.condMat);
            curSetCond = unique(catCondMat(:,1));
            if sum(ismember(setCond, curSetCond))~=5 % if the default set of conditions are there
                setCond = curSetCond;
            end
