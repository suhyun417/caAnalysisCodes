% runSaveCaTS_acrossSessionFOV_RS.m
%
% 2022/11/07 SHP
% save Resting State trial time series for the neurons across sessions
% using a longitudinally aligned membership of the cells
% no averaging, just gathering the time series so that we can link
% responses of same cell from different conditions more easily
...
%         tSeries_RS(iRun).C = neuron.C(:, count+1:count+nFrame);  %
%         
%         spatial_RS(iRun).A = neuron.A;
%         spatial_RS(iRun).batches = neuron.batches;
%         
%         resultsDFL(iS).tS_session = tS_session;
    ...
    for iCell = 1:size(cellIDAcrossDay, 1)
        ...
        for iSetCell = 1:length(curCells_id)
            ...
            cellTS_RS(iCell, curCells_session(iSetCell)).C_raw = resultsRS(curCells_session(iSetCell)).C_raw(curCells_id(iSetCell), :);
            cellTS_RS(iCell, curCells_session(iSetCell)).C = resultsRS(curCells_session(iSetCell)).C(curCells_id(iSetCell), :);
            
%             % RS snr for later use
...
%         cellPix(iCell).contourCell = contourCell;
    ...
    %%
    if flagSaveFile
        fname_caTSFOV = fullfile(dirProcdata, sprintf('_marmoset/invivoCalciumImaging/%s/FOV%d/%s_FOV%d_RSsorted.mat', nameSubj, FOV_ID, nameSubj, FOV_ID));
        save(fname_caTSFOV, 'cellTS_RS', 'resultsRS')
